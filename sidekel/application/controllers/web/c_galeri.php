...
class C_galeri extends CI_Controller {
	...
	function index()
    {
    	...
    	//pagination
		$config['base_url'] =base_url().'/web/c_galeri/index';
		$config['total_rows'] = $this->m_galeri->getAllGaleriNumRows(); //$this->db->get('tbl_berita')->num_rows();
		$config['per_page'] = '8';
		$config['num_links'] = 3;
		...
		$pag = $this->db->get($this->m_galeri->getAllGaleri(), $config['per_page'], $this->uri->segment(4));  //$this->m_berita->berita_all( $config['per_page']); //      
		//$pag = $this->m_berita->berita_all( 6,4); //      
		$data['konten_galeri'] = $pag->result();
		
		$data['logo'] = $this->load->view('v_logo', $data, TRUE);
		$data['menu'] = $this->load->view('v_navbar', $data, TRUE);
		$data['content'] = $this->load->view('web/galeri',$data,TRUE);
		$temp['footer'] = $this->load->view('v_footer',$data,TRUE);
		//echo json_encode($this->m_galeri->getAllGaleriNumRows());
