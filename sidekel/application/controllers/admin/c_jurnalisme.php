...
class C_jurnalisme extends CI_Controller {
    ...
    function load_data() {
		...
		foreach ($records['records']->result() as $row)
		{
			...
			$record_items[] = array(
				...
				<button data-toggle="modal" href="#dialog-print" type="submit" class="btn btn-primary btn-xs" title="Lihat Detail Berita" onclick="show_detail(\''.$row->id_berita.'\')"/><i class="fa fa-eye"></i></button>
				'
			);  
		}
		//Print please
	...
	$url='web/c_home/get_detail_berita/';
        ...
        foreach($post as $id){
			...
			$thumb = $this->m_berita->getThumbByIdBerita($id);
            $this->m_berita->deleteBerita($id,$gambar,$thumb);
	...
	function show_detail($id){
		...
		if($this->session->userdata('logged_in') AND $role == 'Administrator')
		{
			...
			$data['berita'] = $this->m_jurnalisme->getBeritaByIdberita($id);        		
			$data['page_title'] = 'DETIL BERITA';
			$data['content'] = $this->load->view('jurnalisme/v_detil', $data);
