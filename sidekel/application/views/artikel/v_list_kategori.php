...
function edit_kategori(id) {
  window.location = _base_url + 'admin/c_artikel/edit_kategori/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'admin/c_artikel/add_kategori';
    }	
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("admin/c_artikel/delete_kategori/");?>",
					   data: "items="+itemlist,
...
<h3><?= $page_title ?></h3>
<div class="row">
	<input type="button" class="btn btn-flat btn-danger pull-right" value="Kembali" id="kategori" onclick="location.href='<?= base_url() ?>admin/c_artikel'"/>
</div>
