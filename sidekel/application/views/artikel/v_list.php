...
function edit_artikel(id) {
  window.location = _base_url + 'admin/c_artikel/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'admin/c_artikel/add';
    }	
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("admin/c_artikel/delete/");?>",
					   data: "items="+itemlist,
...
<h3><?= $page_title ?></h3>
<div class="row">
	<input type="button" class="btn btn-flat btn-success pull-right" value="Tambah Kategori" id="kategori" onclick="location.href='<?= base_url() ?>admin/c_artikel/lists_kategori'"/>
</div>
