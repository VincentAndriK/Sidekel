...
function edit_indikator_kesejahteraan(id) {
  window.location = _base_url + 'indikatorkesejahteraan/c_indikator_kesejahteraan/edit/' + id;
}

function hapus_indikator(id) {
	if(confirm('Apakah anda yakin ingin menghapus data ini ? '))
	{
		window.location = _base_url + 'indikatorkesejahteraan/c_indikator_kesejahteraan/delete/' + id;
	}
	else
	{
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'indikatorkesejahteraan/c_indikator_kesejahteraan/add';
    }	
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("indikatorkesejahteraan/c_indikator_kesejahteraan/delete/");?>",
					   data: "items="+itemlist,
