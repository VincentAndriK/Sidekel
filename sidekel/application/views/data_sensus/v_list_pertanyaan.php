...
function edit_pertanyaan_sensus(id) {
  window.location = _base_url + 'indikatorkesejahteraan/c_pertanyaan_sensus/edit/' + id;
}

function hapus_pertanyaan(id) {
	if(confirm('Apakah anda yakin ingin menghapus data ini ? '))
	{
		window.location = _base_url + 'indikatorkesejahteraan/c_pertanyaan_sensus/delete/' + id;
	}
	else
	{
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("indikatorkesejahteraan/c_pertanyaan_sensus/delete/");?>",
					   data: "items="+itemlist,
	...
	<div class="col-md-3">
		...
		<li class="dropdown">
			...
			<ul class="dropdown-menu dropdown-user">
				<li>
					<a href="<?php echo site_url('indikatorkesejahteraan/c_pertanyaan_sensus/ExportToExcel/');?>"><i class="fa fa-download fa-fw"></i> Export ke Excel</a>
				</li>
				<li>
					<a href="<?php echo site_url('indikatorkesejahteraan/c_pertanyaan_sensus/ImportToExcel/');?>"><i class="fa fa-upload fa-fw"></i> Import dari Excel</a>
				</li>
			</ul>
		</li>
	</ul> 
