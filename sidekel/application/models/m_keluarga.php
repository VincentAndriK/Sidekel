...
/* 	//langkah 1
  ...
  function getAll()
  {
	...
	$this->db->where('tbl_keluarga.koordinat_polygon !=','');
	$this->db->group_by('tbl_keluarga.id_keluarga');
	//$this->db->order_by('tbl_keluarga.id_keluarga desc');
	$q = $this->db->get();
	return $q->result(); 
  }
  ...
  {
	$this->db->select('
	tbl_keluarga.koordinat_marker as marker,
	')
	->from('tbl_keluarga');
	$this->db->join('tbl_hasil_sensus','tbl_hasil_sensus.id_keluarga = tbl_keluarga.id_keluarga');
	$this->db->where('tbl_hasil_sensus.is_new', 0);
  ...
  function getCount()
	{
	$query = $this->db->query('select count(tbl_hub_kel.id_keluarga) as jumlah from tbl_hub_kel join tbl_hasil_sensus on tbl_hub_kel.id_keluarga = tbl_hasil_sensus.id_keluarga join tbl_keluarga on tbl_hasil_sensus.id_keluarga = tbl_keluarga.id_keluarga where tbl_hasil_sensus.is_new = 0 and tbl_keluarga.koordinat_polygon != "" group by tbl_hasil_sensus.id_keluarga');
	return $query->result();
	}
	...
	function getStatus()
	{
	$query = $this->db->query('select max(tbl_hasil_sensus.status) as status from tbl_hasil_sensus  join tbl_keluarga on tbl_hasil_sensus.id_keluarga = tbl_keluarga.id_keluarga where tbl_hasil_sensus.is_new = 0 and tbl_keluarga.koordinat_polygon != "" group by tbl_hasil_sensus.id_keluarga');
	return $query->result();
	}
	
	function getAnakMiskin()
	{
	$query = $this->db->query('select count(tbl_hub_kel.id_status_keluarga) as anak from tbl_hub_kel join tbl_hasil_sensus on tbl_hub_kel.id_keluarga = tbl_hasil_sensus.id_keluarga where tbl_hub_kel.id_status_keluarga = 4 and tbl_hasil_sensus.status = "Sangat Miskin"');
	return $query->row()->anak;
	}
	
	function getAnakMiskinlala()
	{
	$query = $this->db->query('select count(tbl_hub_kel.id_status_keluarga) as anak from tbl_hub_kel join tbl_hasil_sensus on tbl_hub_kel.id_keluarga = tbl_hasil_sensus.id_keluarga where tbl_hub_kel.id_status_keluarga = 4 and tbl_hasil_sensus.status = "Miskin"');
	return $query->row()->anak;
	}
   ...
   function getAnak()
	...
	$q = $this->db->get();
	return $q->result(); */  
	$query = $this->db->query('select tbl_hub_kel.id_keluarga, sum(if(tbl_hub_kel.id_status_keluarga = 4,1,0)) as anak from tbl_hub_kel right join tbl_hasil_sensus on tbl_hub_kel.id_keluarga = tbl_hasil_sensus.id_keluarga join tbl_keluarga on tbl_hasil_sensus.id_keluarga = tbl_keluarga.id_keluarga where tbl_hasil_sensus.is_new = 0 and tbl_keluarga.koordinat_polygon != "" group by tbl_hub_kel.id_keluarga');
	return $query->result();
	}
 ...
 /*  function getAngka($id)
  ...
  {	
	...
	function getIdKeluargaByNoKK1($id)
	{	
		$this->db->select('id_keluarga');
		$this->db->where('no_kk', $id);
		$q = $this->db->get('tbl_keluarga');
		return $q->row()->id_keluarga;
	}
