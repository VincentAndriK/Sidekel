...
class M_slider_beranda extends CI_Model {
	...
	{
		$back = $this->getGambarByIdSlider($id,'konten_background');
		if($back != 'uploads/web/slider_beranda/defaultSlider.jpg'){unlink($back);}
		
		$logo = $this->getGambarByIdSlider($id,'konten_logo');
		if($logo != 'uploads/web/slider_beranda/defaultLogo.png'){unlink($logo);}
		
		$this->db->where('id_slider_beranda', $id);
		$this->db->delete($this->_table);
	}
	function getGambarByIdSlider($id,$column)
	{
