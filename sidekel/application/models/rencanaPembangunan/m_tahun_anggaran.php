...
class M_tahun_anggaran extends CI_Model {
  ...
  function __construct()
  {
	...
	function deleteTahunAnggaran($id)
	{
		$this->db->where('id_tahun_anggaran', $id);
		$this->db->delete($this->_table);
	}

	function getTahunAnggaranById($id) //edit
	{	
		return $this->db->get_where($this->_table,array('id_tahun_anggaran' => $id))->row();
	}

	function updateTahunAnggaran($where, $data) //update
	{
		$this->db->where($where);
	...
	function get_tahun_dynamic($id_periode) 
	{      
		$this->db->where('id_periode !=','0');	
		$this->db->where('id_periode',$id_periode);
      	$records = $this->db->get('ref_rp_periode')->row();
   		$data=array();
		for( $i = $records->periode_awal; $i <= $records->periode_akhir; $i++)
		{
			$data[''] = '--Pilih--';
			$data[$i] = $i;
		}
